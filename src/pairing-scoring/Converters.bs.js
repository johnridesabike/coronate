...
function matches2ScoreData(matchList) {
  ...
  var makeScoreData = function (playerId, origRating, newRating, result, oppId, color) {
    ...
    var oldData = match !== undefined ? Caml_option.valFromOption(match) : Scoring$Coronate.createBlankScoreData(playerId);
    var match$1 = oldData.ratings.length;
    ...
    var newOpponentResults = oldData.opponentResults;
    ...
    return {
            colorScores: oldData.colorScores.concat(/* array */[colorToScore(color)]),
            colors: oldData.colors.concat(/* array */[color]),
            id: playerId,
            isDummy: playerId === dummyId,
            opponentResults: newOpponentResults,
            ratings: oldData.ratings.concat(newRatings),
            results: oldData.results.concat(/* array */[result]),
