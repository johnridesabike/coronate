...
export function scoreData2PairingData(playerData, avoidPairs, scoreData) {
    const pairingData = Object.values(scoreData).reduce(
        function (acc, data) {
            const avoidIds = avoidPairs.reduce(
                // flatten the avoidlist
                (acc2, pair) => (pair.includes(data.id))
                    ? acc2.concat(pair)
                    : acc2,
                []
            ).filter(
                // filter the player's own ID from the list
                (id) => id !== data.id
            );
            // `upperHalf` and `isDueBye` default to `false` and will have to be
            // set by another function later.
            const pairData = {
                avoidIds,
                isDueBye: false,
                rating: playerData[data.id].rating,
                // `score` is calculated and cached here because the blossom
                // pairing will reuse it many times.
                score: sum(data.results),
                upperHalf: false
            };
            const updatedPlayer = mergeLeft(data, pairData);
            return acc.concat([updatedPlayer]);
        },
        []
    );
    // TODO: remove this tcomb check for production
