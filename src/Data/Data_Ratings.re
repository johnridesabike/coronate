...
let calcNewRatings =
    (~whiteRating, ~blackRating, ~whiteMatchCount, ~blackMatchCount, ~result) => {
  ...
  let whiteResult =
    Data_Match.Result.toScoreWhite(result)->Data_Scoring.Score.toFloat;
  let blackResult =
    Data_Match.Result.toScoreBlack(result)->Data_Scoring.Score.toFloat;
  (
